...
// =====================
// DATA (KEEP YOUR ARRAY ABOVE THIS)
// =====================
    ...
    <div class="bg-[#DDC7BB] rounded-2xl overflow-hidden shadow-md transition-transform hover:scale-[1.02] duration-500">
        ...
        <div class="px-4 py-3 space-y-4">
            ...
            <p class="text-[#4F3527] font-semibold text-sm">
                Owner — ${hotel.owner.fullName}
            </p>

            <div class="flex justify-between items-center">
                <a href="form/sign.html" class="bg-[#2D1B14] text-white px-4 py-1 rounded text-sm">
                    Sign up
                </a>
                <p class="font-bold text-[#4F3527] text-sm">
                    ${hotel.prices?.currency || ''} ${hotel.prices?.formattedAmount || hotel.formattedAmount || ''}
                </p>
            </div>
        </div>
    </div>
...
function showAllProperties(propertyArr) {
    const hotelCon   = document.getElementById('hotel-con');
    const emptyState = document.getElementById('no-result-property');

    if (!hotelCon) return;

    if (propertyArr.length === 0) {
        hotelCon.innerHTML = "";
        if (emptyState) emptyState.textContent = "No Hotels Found";
        return;
    }

    if (emptyState) emptyState.textContent = "";

    hotelCon.innerHTML = propertyArr
        .slice(0, 9)
        .map(property => hotelCardDisplay(property))
        .join('');
}
